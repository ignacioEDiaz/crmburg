                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-1 px-2 py-2.5 rounded-2xl border transition-all shrink-0 w-[calc((100%-16px)/3)] sm:w-auto sm:min-w-[120px] ${
                      isActive
                        ? 'bg-[#242426] border-primary text-primary shadow-md ring-1 ring-primary/40'
                        : 'bg-[#242426]/60 border-white/10 text-secondary hover:text-white hover:bg-[#2c2c2e]'
                    }`}
                  >
                    {/* Smaller Category Icon */}
                    <span className={`material-symbols-outlined text-[18px] sm:text-[20px] transition-transform ${isActive ? 'text-primary scale-110' : 'text-secondary'}`}>
                      {getCategoryIcon(catName)}
                    </span>

                    {/* Category Title in Uppercase */}
                    <span className={`font-black text-[10px] sm:text-[11px] tracking-wider uppercase text-center truncate w-full ${isActive ? 'text-primary' : 'text-secondary'}`}>
                      {catName}
                    </span>

                    {/* Active Underline Pill */}
                    {isActive && (
                      <span className="w-5 h-0.5 rounded-full bg-primary mt-0.5 shadow-sm"></span>
                    )}
                  </button>
                );
              })}
                ...
                {(offers.length > 0 ? offers : [1]).map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      currentOfferIndex === idx ? 'w-8 bg-primary' : 'w-2 bg-white/20'
                    }`}
                  />
                ))}
              </div>
                ...
                filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`material-symbols-outlined text-[20px] ${isFav ? 'text-primary' : 'text-white'}`}
                      ...
                      <div className="flex flex-col gap-xs pt-3 border-t border-white/10">
                        ...
                        <div className="flex items-center justify-between mt-4">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="font-price-display text-price-display text-primary font-black text-xl">

...
import { Plus, Users, Clock, ShoppingBag, CheckCircle2, Trash2, Printer, Edit, X, Search, Sparkles, ChefHat, DollarSign, CreditCard, QrCode, Zap, User, MessageSquare, Utensils } from 'lucide-react';
...
export default function CrmTablesView() {
  ...
  const [waiterName, setWaiterName] = useState('Sofía');
  ...
  const handleOpenTableModal = (table) => {
    ...
    if (table.activeOrder) {
      ...
      setWaiterName(table.activeOrder.waiter || 'Sofía');
  ...
  const handleConfirmTableOrder = (sendToKitchen = false) => {
    ...
    showToast(`🍽️ ${selectedTable.name} abierta con ${tableCart.length} productos`);
  ...
  const handleCloseAndPayTable = () => {
    ...
    showToast(`⚡ ${selectedTable.name} cobrada ($${finalTotal.toFixed(2)}) y liberada 🟢`);
  ...
  // Filter products for table order taking
  const filteredProducts = products.filter(p => {
    if (!p) return false;
    const pName = (p.name || '').toLowerCase();
    const matchesSearch = pName.includes(productSearch.toLowerCase());
    if (selectedCat === 'Todas') return matchesSearch;
    return matchesSearch && p.category === selectedCat;
  });
  ...
  return (
    <div className="flex flex-col w-full gap-lg font-sans">
      ...
      {/* ================= TABLE ORDER & ACCOUNT MODAL / DRAWER ================= */}
      {selectedTable && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl max-h-[90vh] bg-[#18181b] border border-white/15 rounded-3xl p-6 shadow-2xl text-white flex flex-col justify-between overflow-hidden">
            ...
            {/* Modal Body: Grid split (Products catalog on left + Table order on right) */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-md py-4 overflow-y-auto hide-scrollbar flex-1">
              
              {/* LEFT SIDE: PRODUCT SELECTION MENU (7 COLS) */}
              <div className="md:col-span-7 flex flex-col gap-3">
                <h4 className="font-extrabold text-white text-xs uppercase tracking-wider">Cargar Productos a la Mesa:</h4>
                ...
                {/* Products Cards */}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[40vh] overflow-y-auto hide-scrollbar pr-1">
                  {filteredProducts.map(p => (
                    <div
                      key={p.id}
                      onClick={() => addToTableCart(p)}
                      className="bg-[#242426] hover:bg-[#2c2c30] border border-white/10 rounded-2xl p-2.5 flex flex-col justify-between cursor-pointer active:scale-95 transition-all text-xs"
                    >
                      <img src={p.image || '/images/burger-supreme.jpg'} alt={p.name} className="h-16 object-contain mx-auto my-1" />
                      <p className="font-bold text-white truncate text-[11px]">{p.name}</p>
                      <p className="font-mono text-[#ffb700] font-black mt-1">${Number(p.price || 0).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              </div>

              {/* RIGHT SIDE: TABLE ACCOUNT FORM & SUMMARY (5 COLS) */}
              <div className="md:col-span-5 bg-[#242426] border border-white/10 rounded-2xl p-4 flex flex-col justify-between space-y-3 text-xs">
                <div>
                  ...
                  {/* Cart Items List */}
                  <div className="space-y-1.5 max-h-[25vh] overflow-y-auto hide-scrollbar pr-1 border-t border-white/10 pt-2">
                    {tableCart.length === 0 ? (
                      <p className="text-neutral-500 font-bold text-center py-6">No hay productos cargados en esta mesa.</p>
                    ) : (
                      tableCart.map((item, idx) => (
                        <div key={idx} className="bg-[#18181b] p-2 rounded-xl border border-white/5 flex items-center justify-between">
                          <div className="min-w-0 pr-2">
                            <p className="font-bold text-white truncate">{item.name}</p>
                            <p className="font-mono text-[#ffb700] text-[11px] font-black">${((item.price || 0) * item.quantity).toFixed(2)}</p>
                          </div>

                          <div className="flex items-center gap-1 shrink-0 bg-[#242426] p-1 rounded-lg">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-md border-b border-white/10 pb-4">
        <div>
          ...
          <p className="text-xs text-neutral-400 font-bold">Control en tiempo real de mesas en Planta Baja y Planta Alta</p>
        </div>

        {/* Floor Switcher & Add Table Button */}
        <div className="flex items-center gap-3">
          
          {/* Planta Switcher */}
          <div className="flex items-center gap-1.5 bg-[#242426] p-1.5 rounded-2xl border border-white/10">
            <button
              onClick={() => setActiveFloor('Planta Baja')}
              className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${
                activeFloor === 'Planta Baja' ? 'bg-[#ffb700] text-black shadow-md' : 'text-neutral-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-black transition-all ${
                activeFloor === 'Planta Alta' ? 'bg-[#ffb700] text-black shadow-md' : 'text-neutral-400 hover:text-white'
              }`}
            >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs uppercase tracking-wider flex items-center gap-1.5 shadow-lg transition-all"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-md">
        <div className="bg-[#18181b] border border-white/10 p-4 rounded-3xl flex items-center justify-between">
          <div>
          ...
          <Utensils className="w-8 h-8 text-neutral-500" />
        </div>

        <div className="bg-[#18181b] border border-emerald-500/20 p-4 rounded-3xl flex items-center justify-between">
          <div>
        ...
        <div className="bg-[#18181b] border border-rose-500/20 p-4 rounded-3xl flex items-center justify-between col-span-2 sm:col-span-1">
          <div>
        ...
        {floorTables.map((table) => {
          ...
          return (
            <div
              ...
              className={`relative bg-[#18181b] border rounded-3xl p-5 flex flex-col justify-between cursor-pointer transition-all duration-200 shadow-xl hover:scale-102 active:scale-98 select-none ${
                isFree
                  ? 'border-emerald-500/40 hover:border-emerald-400 hover:shadow-emerald-500/10'
                  : 'border-rose-500/60 bg-rose-950/10 hover:border-rose-400 hover:shadow-rose-500/10'
              }`}
            >
              {/* Status Badge */}
              <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-3">
                <span className="font-black text-white text-lg tracking-tight">{table.name}</span>
              ...
              {/* Table Info & Active Order details */}
              <div className="space-y-2 my-2">
                ...
                {!isFree && table.activeOrder && (
                  <div className="bg-[#242426] p-3 rounded-2xl border border-white/10 space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-[#ffb700] font-black">{table.activeOrder.code}</span>
                      <span className="text-[10px] text-neutral-400">Hora: {table.activeOrder.openTime}</span>
                    </div>

                    <p className="text-white font-bold text-[11px]">Mozo: {table.activeOrder.waiter}</p>

                    <div className="flex items-center justify-between border-t border-white/5 pt-1.5 mt-1">
                      <span className="text-[11px] text-neutral-400">Total consumido:</span>
                      <span className="font-mono font-black text-emerald-400 text-sm">${table.activeOrder.total.toFixed(2)}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="pt-3 border-t border-white/10 mt-2">
                <button
                  className={`w-full py-2.5 rounded-2xl font-black text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 ${
                    isFree ? 'bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500 hover:text-black' : 'bg-[#ffb700] text-black hover:bg-yellow-300 shadow-md'
                  }`}
                >
                  {isFree ? '➕ Abrir y Tomar Pedido' : '💳 Gestionar y Cobrar'}
                </button>
              </div>
